  ...
  module Admin
    describe ConstraintsController, type: :controller do
      ...
      describe "GET #new" do
        ...
        context "when space is an initiative" do
          let!(:process) { create :initiative, organization: organization }
          let!(:component) { create :component, participatory_space: process }

          it "has helper with existing participatory spaces" do
            expect(controller.helpers.participatory_spaces_list(:initiatives)).to eq(spaces)
          end

          it "has helper with existing components" do
            expect(controller.helpers.components_list(:initiatives, process.slug)).to eq(components)
          end
        end
