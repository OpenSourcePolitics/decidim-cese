...
describe Decidim::Initiatives::ContentBlocks::HighlightedInitiativesCell, type: :cell do
  ...
  context "when the content block has customized the max results setting value" do
    ...
    it "shows up to 8 initiatives" do
      expect(highlighted_initiatives).to have_selector("a.card--initiative", count: 7)
    end
  end
  ...
  context "when the content block has customized the sorting order" do
    ...
    context "when sorting by most popular" do
      let(:settings) do
        {
          "order" => "most_popular"
        }
      ...
      it "shows up initiatives ordered by supports count" do
        expect(subject.highlighted_initiatives.first).to eq(most_popular_initiative)
      end
    end
  end
end
