  ...
  describe UpdateOrganizationAppearance do
    describe "call" do
      ...
      let(:params) do
        {
          organization: {
            ...
            official_cachet: upload_test_file(Decidim::Dev.test_file("city.jpeg", "image/jpeg")),
            official_signature: upload_test_file(Decidim::Dev.test_file("city.jpeg", "image/jpeg"))
          }
        }
      end
      let(:context) do
      ...
      describe "when the form is valid" do
        ...
        context "when there's a cachet and signature in the params" do
          it "does set a cachet and signature for the organization" do
            command.call
            organization.reload

            expect(organization.attached_uploader(:official_cachet)).to be_present
            expect(organization.attached_uploader(:official_signature)).to be_present
          end
        end
      end
    end
